...
use sip_domain::{
    ...
    id::{InspectionChecklistItemId, InspectionId, WorkOrderId},
...
pub async fn get_inspection(
    ...
    let inspection_id = id.parse::<InspectionId>().map_err(|e| {
        (
            StatusCode::BAD_REQUEST,
            Json(json!({"error": {"code": "BAD_REQUEST", "message": e.to_string()}})),
        )
    ...
    match service.get(&ctx, inspection_id).await {
        ...
        Ok(None) => Err((
            ...
            Json(json!({"error": {"code": "NOT_FOUND", "message": "Inspection not found"}})),
        )),
        Err(e) => Err((
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error": {"code": "INTERNAL_ERROR", "message": e.to_string()}})),
...
pub async fn update_checklist_items(
    ...
    let items: Vec<UpdateChecklistItemInput> = req
        .iter()
        .map(|item| {
            let item_id = item
                .id
                .parse::<InspectionChecklistItemId>()
        ...
        .map_err(|e| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": {"code": "BAD_REQUEST", "message": e}})),
            )
        })?;
